...
from app.textbook.models import (
    Base, Unit, Vocab, Sentence, SentenceVocab, WordType,
    GrammarTip, SentenceGrammar, Question, FitbQuestion,
)
import json

# Point this at wherever your app's textbook DB should live, e.g.
#   sqlite:////absolute/path/to/textbook.db
# Swap for Postgres/MySQL later just by changing this URL -- nothing else
# in these scripts is SQLite-specific.
...
def upsert_question(db: Session, unit_number: int, question_type: str, question_text: str,
                     answer_text: str, vocab_id: int = None, legacy_id: str = None) -> Question:
    ...
    if row is None:
        ...
        db.add(row)
        db.flush()
    elif vocab_id is not None and row.vocab_id is None:
...
def get_all_vocab_with_status(db: Session) -> tuple[dict[str, Vocab], set[str]]:
    ...
    - needs_retry_set is {hanzi} for any word with UNKNOWN_PINYIN or UNKNOWN_ENGLISH
    ...
    needs_retry = {
        ...
        if "UNKNOWN_PINYIN" in (v.pinyin or "") or "UNKNOWN_ENGLISH" in (v.english or "")
    }
