...
Mandarin textbook PDF using two separate Claude calls per unit:
  ...
  2. JSON step: takes that plain-text transcription and structures it into
     vocab/grammar/sentences/fill_in_the_blank/proper_nouns.

The system prompts (SOPs) are identical across all units, so prompt caching
is used to avoid re-sending them on every call — ~90% token savings on units 4+.
"""
...
if not api_key:
    raise ValueError("API Key not found! Did you forget to set CLAUDE_API_KEY?")
...
def call_claude_for_ocr(ocr_system_prompt: str, pdf_bytes: bytes, unit_number: int) -> str:
    """Upload unit pages and return plain-text transcription. SOP is cached."""
    ...
    try:
        ...
        if CLEANUP_UPLOADED_FILES:
            try:
                ...
                print(f"  (warning: failed to delete uploaded file {file_upload.id}: {e})")

    return response.content[0].text.strip()


def call_claude_for_json(json_system_prompt: str, transcription: str) -> dict:
    """Structure a transcription into JSON. SOP is cached."""
    ...
    if response.stop_reason == "max_tokens":
        raise RuntimeError("Response truncated — increase JSON_MAX_TOKENS and retry.")
...
def main():
    ...
    if TESTING:
        print(f"TESTING mode — only processing unit {unit_ranges[0][0]}\n")
    ...
    print(f"Output: {os.path.join(OUTPUT_FILEPATH, OUTPUT_FILENAME)}")
